...
@pytest.mark.parametrize(
    argnames="date_created, expected_data",
    argvalues=[
        # test case when table is empty and unix time is returned
        [
            datetime(1970, 1, 1, 2, 0),
            [
                [datetime(2022, 3, 1, 1, 0, 0), 1, 12.8, 190, 2.5,datetime(2024, 3, 1, 1, 0, 0)],
                [datetime(2022, 3, 1, 1, 0, 0),2, 13.6, 12, 2.0, datetime(2024, 3, 1, 1, 0, 0)],
                [datetime(2022, 3, 1, 1, 0, 0), 3, 10.8, 335, 2.9, datetime(2024, 3, 1, 1, 0, 0)],
            ]
        ],
...
def test_filter_for_newly_arrived_data_target_table_is_empty(
    spark_session, monkeypatch: MonkeyPatch, date_created, expected_data
):

    monkeypatch.setattr(
        "silver_wind_turbine_data_pipeline.SparkSession.sql",
        lambda x, y: spark_session.createDataFrame([[date_created]], "col1 timestamp"),
    )
    ...
    schema = "timestamp timestamp, turbine_id integer, wind_speed double, wind_direction integer, power_output double, date_created timestamp"
    input_df = spark_session.createDataFrame(
        input_data,
        schema,
    )
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        [[1.0, 2.5], [2.2, 2.5], [3.1, 2.5], [4.5, 2.5], [5.0, 2.5], [None, 2.5]],
        # test case when dataset does not have null and nothing should be changed
        [[1.0, 2.5], [2.2, 2.5], [3.1, 2.5], [4.5, 2.5], [5.0, 2.5], [2.5, 2.5]],
    ],
)
def test_fill_nulls_with_mean_value(spark_session, input_data):
    ...
    input_df = spark_session.createDataFrame(
        input_data,
        "power_output double, power_output_mean double",
    )
...
def test_add_mean_and_std_dev_to_df(spark_session):
    ...
    expected_df = spark_session.createDataFrame(
        [
            [1, 2.5, 0.14142135623730964, 2.6],
            [1, 2.7, 0.14142135623730964, 2.6],
            [2, 1.5, 1.4142135623730951, 2.5],
            [2, 3.5, 1.4142135623730951, 2.5],
        ],
        "turbine_id integer, power_output double, power_output_std_dev double, power_output_mean double",
    )
...
def test_add_upper_and_lower_boundaries_of_2_std_dev_to_df(spark_session):
    ...
    expected_df = spark_session.createDataFrame(
        [
            [1, 2.5, 2.6, 0.2, 3.0, 2.2],
            [1, 2.7, 2.6, 0.2, 3.0, 2.2],
            [2, 1.5, 2.0, 1.0, 4.0, 0.0],
            [2, 3.5, 2.0, 1.0, 4.0, 0.0],
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        turbine_id integer, 
        power_output double, 
        power_output_mean double,
        power_output_std_dev double, 
        power_output_2_stddev_upper double, 
        power_output_2_stddev_lower double
        """,
